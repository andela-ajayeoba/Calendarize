...
************************************************/
    ...
    $scope.listProjects = function () {
      $scope.projects = Projects.query();  // console.log($scope.projects);
    };
    /************************************************
...
************************************************/
    ...
    $scope.createTask = function (data) {
      ...
      task.$save(function (response) {
        ...
        uiItem.row.updateVisibleTasks();  // console.log(uiItem);
      }, function (errorResponse) {
        $scope.error = errorResponse.data.message;
      });
    };
...
************************************************/
    ...
    $scope.$on(GANTT_EVENTS.READY, function () {
      $scope.addSamples();  /*$timeout(function() {
...
angular.module('calendarizes').controller('userLoginController', [
  '$scope',
  '$rootScope',
...
angular.module('calendarizes').factory('Persons', [
  '$resource',
  function ($resource) {
    return $resource('persons/:personId', { personId: '@_id' }, { update: { method: 'PUT' } });
  }
]);
angular.module('calendarizes').factory('Projects', [
  '$resource',
  function ($resource) {
    return $resource('projects/:projectId', { projectId: '@_id' }, { update: { method: 'PUT' } });
  }
]);
angular.module('calendarizes').factory('Tasks', [
  '$resource',
  function ($resource) {
    return $resource('tasks/:taskId', { taskId: '@_id' }, { update: { method: 'PUT' } });
  }
]);// 'use strict';
// angular.module('calendarizes').service('$loginModal',function($modal){
//     var myModal = $modal({template:'user-login-modal.html', show:false});
//     this.show=function(){
